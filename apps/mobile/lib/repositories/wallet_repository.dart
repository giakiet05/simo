...
class WalletRepository {
  ...
  Future<void> updateWallet(Wallet wallet) async {
    ...
    await db.transaction((txn) async {
      if (wallet.isDefault) {
        await txn.update('wallets', {'is_default': 0, 'synced': 0, 'updated_at': DateTime.now().toUtc().toIso8601String()});
      }
  ...
  Future<void> deleteWallet(String id) async {
    ...
    await db.transaction((txn) async {
      ...
      await txn.update(
        'transactions',
        {'wallet_id': null, 'synced': 0, 'updated_at': nowIso},
        where: 'wallet_id = ?',
        whereArgs: [id],
      );

      // Delete associated transfers and record tombstones
      ...
      // Delete wallet and record tombstone
      await txn.delete(
        'wallets',
        where: 'id = ?',
        whereArgs: [id],
      ...
      if (wasDefault) {
        ...
        if (remaining.isNotEmpty) {
          await txn.update(
            'wallets',
            {'is_default': 1, 'synced': 0, 'updated_at': nowIso},
            where: 'id = ?',
  ...
  Future<void> setDefaultWallet(String id) async {
    ...
    final nowIso = DateTime.now().toUtc().toIso8601String();
    await db.transaction((txn) async {
      await txn.update('wallets', {'is_default': 0, 'synced': 0, 'updated_at': nowIso});
      await txn.update(
        'wallets',
        {'is_default': 1, 'synced': 0, 'updated_at': nowIso},
        where: 'id = ?',
